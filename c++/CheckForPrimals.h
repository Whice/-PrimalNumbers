...
public:	static _inline bool IsPrimal(uint64_t number, Memory &memoryForKeeper)
	{
...
public:	static _inline void RangeCheck(uint64_t &numberStart, const uint64_t &range, Memory &memoryForKeeper)
	{
...
public:	static _inline vector<uint64_t>* GetListPrimalNumbers(uint64_t &numberStart, const uint64_t &range, Memory &memoryForKeeper)
	{
		//Посичтать максимум
		uint64_t numberMax = numberStart + range;
